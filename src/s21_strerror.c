...
#if defined(__linux__)
  static const char *error_messages[ERROR_COUNT] = {LINUX_ERRORS};
#elif defined(__APPLE__)
  static const char *error_messages[ERROR_COUNT] = {MAC_ERRORS};
#endif
  static char result[256];

  if (errnum >= 0 && errnum < ERROR_COUNT) {
    s21_sprintf(result, "%s", error_messages[errnum]);
  } else {
